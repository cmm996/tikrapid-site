...
后台支持给每条 IP / CIDR 设置 `business_type` 业务类型、`price` 续费价格、`contact` 客户联系方式、`source` IP 段来源、`entry_node` 入口节点、`transit_node` 中转节点、`landing_node` 落地节点、`landing_country` 落地国家、`expires_at` 到期时间。到期时间格式为 `YYYY-MM-DD`，不填写表示长期有效。到期后的记录会在后台显示为“已过期 / 自动停用”，并且 `/api/ip/check` 不会继续放行。

顶部“本月续费金额”会按本月到期的记录汇总 `price` 字段里的金额数字，用来快速查看本月应续费金额。

后台列表会显示剩余天数，并提供续费提醒颜色：
